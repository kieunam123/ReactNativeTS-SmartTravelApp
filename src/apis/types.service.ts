...
export interface IGoogleImgResult {
  knowledge_graph?: IImageInformation[];
  visual_matches?: IVisualMatches[];
}
...
export interface IImageInformation {
  ...
  thumbnail?: string;
  link?: string;
  images?: IImageInfoList[];
}

export interface IImageInfoList {
  title?: string;
  source?: string;
  link?: string;
}

export interface IVisualMatches {
  position: string;
  title: string;
  link: string;
  thumbnail: string;
...
export interface IGoogleResult {
  search_metadata?: ISeachMetaData;
  search_parameters?: ISearchParameter;
  knowledge_graph?: ISearchInformation;
  top_sights?: ITopSights;
  inline_images?: ILocationImage[];
  organic_results?: IResultInfo[];
  error?: string;
  lat?: number;
  long?: number;
}

export interface ITopSights {
  sights?: ISights[];
...
export interface ISeachMetaData {
  id: string;
  status: string;
...
export interface ISearchInformation {
  title?: string;
  type?: string;
  header_images?: IHeaderImageInfo[];
  description?: string;
  local_map?: ILocationMapInfo;
  weather?: string;
  population?: string;
  area_code?: string;
  climate?: string;
...
export interface IHeaderImageInfo {
  image: string;
  source: string;
}

export interface ILocationMapInfo {
  image: string;
  link: string;
}
...
export interface IChatGPTCall {
  id: string;
  object: string;
  created: number;
  model: string;
  choices: IChatGPTResponse[];
}
